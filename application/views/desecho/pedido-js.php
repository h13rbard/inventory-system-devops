...
table = $("#dtRegistros").DataTable({
        ...
        ajax: {
            "url": "<?=base_url()?>desecho/productos",
            "type": "POST",
        },
        columns: [ 
            { data: "id" },
...
{
	// $.ajax({
    // url : "<?php echo site_url('desecho/checar_exis/')?>" + venta_id,
    // type: "GET",
    // dataType: "JSON",
    // success: function(data)
    // {
    //     if (data.resultado) {
			window.open('<?=base_url()?>desecho/confirmar/'+venta_id,"_self");
			window.open('<?=base_url()?>desecho/ticket/'+venta_id,'_blank');
	// 	} else {
...
function agregar(id)
{
	...
	$.ajax({
		url : "<?php echo site_url('desecho/add_partida/')?>" + venta_id + "/" + id,
		type: "GET",
		dataType: "JSON",
		success: function(data)
		{
			...
			if (data.nuevo) {
				...
				'<td><a href="'+url_+'desecho/del_partida/'+venta_id+'/'+data.partida_id+'" class="btn btn-sm btn-danger">Eliminar</a></td>'+
				'</tr>';
...
$('#partidas').on('keypress', $('.cantidad'), function(event){
    ...
    if(keycode == '13'){
		...
		// alert($('#'+event.target.id).data('partida'))
			...
			$.ajax({
				url : "<?php echo site_url('desecho/update_partida')?>",
				type: "POST",
...
$('#encabezado #cliente').on('keypress', $('#cliente'), function(event){
    ...
    if(keycode == '13'){
		$.ajax({
			url : "<?php echo site_url('desecho/update_cliente')?>",
			type: "POST",
...
$('#encabezado #barcode').on('keypress', $('#barcode'), function(event){
    ...
    if(keycode == '13'){
		...
		$.ajax({
		url : "<?php echo site_url('desecho/add_codigo')?>",
		type: "POST",
...
$('#encabezado #pago').on('change', $('#pago'), function(event){
	$.ajax({
		url : "<?php echo site_url('desecho/update_pago')?>",
		type: "POST",
