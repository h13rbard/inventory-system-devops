...
table = $("#dtRegistros").DataTable({
        ...
        ajax: {
            "url": "<?=base_url()?>almacen/datatable",
            "type": "POST",
        },
        columns: [ 
            { data: "id" },
...
{
	// if (id == 1) {
	// 	toastr.error("El cliente PUBLICO GENERAL no puede ser modificado.");
	// 	return;
	// }
    ...
    $.ajax({
    url : "<?php echo site_url('almacen/ajax_edit/')?>" + id,
    type: "GET",
    dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add')
    {
    url = "<?php echo site_url('almacen/ajax_add')?>";
    }
    else
    {
    url = "<?php echo site_url('almacen/ajax_update')?>";
    }
