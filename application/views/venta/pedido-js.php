...
var cliente_id = <?=$pedido->cliente_id?>;
var tableBusqueda;
...
tableBusqueda = $("#dtBusqueda").DataTable({
        processing: true,
        serverSide: true,
        keys: true,
        "iDisplayLength": 5,
        // stateSave: true,
        ajax: {
            "url": "<?=base_url()?>clientes/busqueda",
            "type": "POST",
        },
        columns: [ 
            { data: "id" },
...
{
	$.ajax({
    url : "<?php echo site_url('venta/checar_exis/')?>" + venta_id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
        if (data.resultado) {
			window.open('<?=base_url()?>venta/confirmar/'+venta_id,"_self");
			window.open('<?=base_url()?>venta/ticket/'+venta_id,'_blank');
		} else {
...
function agregar(id)
{
	...
	$.ajax({
		url : "<?php echo site_url('venta/add_partida/')?>" + venta_id + "/" + id,
		type: "GET",
		dataType: "JSON",
		success: function(data)
		{
			...
			if (data.nuevo) {
				...
				'<td><a href="'+url_+'venta/del_partida/'+venta_id+'/'+data.partida_id+'" class="btn btn-sm btn-danger">Eliminar</a></td>'+
				'</tr>';
...
$('#partidas').on('keypress', $('.cantidad'), function(event){
    ...
    if(keycode == '13'){
		...
		// alert($('#'+event.target.id).data('partida'))
			...
			$.ajax({
				url : "<?php echo site_url('venta/update_partida')?>",
				type: "POST",
...
$('#encabezado #cliente').on('keypress', $('#cliente'), function(event){
    ...
    if(keycode == '13'){
		$.ajax({
			url : "<?php echo site_url('venta/update_cliente')?>",
			type: "POST",
...
$('#encabezado #pago').on('change', $('#pago'), function(event){
	$.ajax({
		url : "<?php echo site_url('venta/update_pago')?>",
		type: "POST",
...
$('#encabezado #barcode').on('keypress', $('#barcode'), function(event){
    ...
    if(keycode == '13'){
		...
		$.ajax({
		url : "<?php echo site_url('venta/add_codigo')?>",
		type: "POST",
...
function seleccionar(nuevo_cliente) {
	$('#modal_busqueda').modal('hide');	
	cliente_id = nuevo_cliente;
	$.ajax({
		url : "<?php echo site_url('venta/cambiar_cliente')?>",
		type: "POST",
