...
class Formaspago extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('formaspago/index', $data);
		$this->load->view('layout/footer');
		$this->load->view('formaspago/index-js', $data);
    ...
    public function ajax_add()
    {
        ...
        {    
            $r = $this->Formapago_model->existeNombre(0, $this->input->post('nombre') );
            if ($r > 0)
            {
            ...
            $insert = $this->Formapago_model->insert();
    ...
    {
        $this->load->model('Formapago_model');
        $data = $this->Formapago_model->getById($id);
    ...
    public function ajax_update()
    {
        ...
        {    
            $r = $this->Formapago_model->existeNombre($this->input->post('id'), $this->input->post('nombre') );
            if ($r > 0)
            {
                echo json_encode(array("status" => FALSE, "mensaje" => "El campo Nombre debe contener un valor único." ));
                exit();
            ...
            $update = $this->Formapago_model->update($this->input->post('id'));
            echo json_encode(array("status" => TRUE, "mensaje" => "Registro guardado correctamente."));
        }
    }
